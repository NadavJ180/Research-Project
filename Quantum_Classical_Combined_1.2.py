...
# 1.1 -> 1.2
# Removed all SI units, now only option is standardized units.
# Removed: KB_SI, E0_J, T_scale_factor, T_K_arr
# Now T_arr = 1 /beta, no unit conversion.
# Input energy levels are dimentionless (n² or n+0.5, etc.) 
    ...
    if stop_reason == "converged":
        ...
        if plat_end is not None:
            ...
            while plat_start > 1 and pre_d[plat_start-1] is not None and pre_d[plat_start-1] < tol_xi:
                plat_start -= 1
            # Take 3 points from the middle of the plateau (not the edge near collapse)
...
# ── Main Cv(T) figure ────────────────────────────────────────────────────────
def plot_cv_curves(T_arr, cv_quantum, cv_numerical_classical,
    ...
    ax1.plot(T_arr, cv_numerical_classical, color=GREEN, linewidth=2, linestyle="--",
...
def run(energies, system_name,
        ...
        xi_start=3.0, tol_xi=1e-3, min_stable_xi=5, xi_multiplier=1.3, max_xi_steps=80,
        tol_cv=1e-4, min_stable_n=3,
        cv_analytic=None, T_units_label=r"$k_B T \,/\, E_0$"):

    beta_arr = np.linspace(beta_min, beta_max, n_beta)
    T_arr    = 1.0 / beta_arr   # dimensionless: kBT/E0
    ...
    if valid_xi_mask.any():
        ...
        plot_xi_convergence_diagnostic(sweep["xi_results"][idx_hard_xi],
                                       beta_arr[idx_hard_xi], T_arr[idx_hard_xi],
                                       tol_xi, system_name)
    ...
    if valid_n_mask.any():
        ...
        if nr_hard is not None:
            plot_n_convergence_diagnostic(nr_hard, beta_arr[idx_hard_n], T_arr[idx_hard_n],
                                          tol_cv, system_name)

    plot_cv_curves(T_arr, cv_quantum, cv_classical, xi_conv, n_conv, system_name,
    ...
    return {"beta_arr": beta_arr, "T_arr": T_arr,
