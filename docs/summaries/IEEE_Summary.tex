...
\subsubsection{Why this is really $\hbar\to0$: a semiclassical picture}
The Bohr--Sommerfeld quantization rule states that the number of bound levels below energy $E$ is, to leading order, the classical phase-space area enclosed by the constant-energy orbit, measured in units of $2\pi\hbar$:
\begin{equation}
n(E) \;\approx\; \frac{1}{2\pi\hbar}\oint_{H(x,p)=E} dx\,dp .
\label{eq:bohrsommerfeld}
\end{equation}
The classical orbit itself -- and hence the phase-space area in Eq.~\eqref{eq:bohrsommerfeld} -- does not depend on $\hbar$; only the \emph{size of the cell} used to count states does. Shrinking $\hbar\to\hbar/\xi$ therefore packs $\xi$ times as many levels into the same energy window, i.e. it shrinks the local level spacing as $\Delta E(\xi)\sim\Delta E/\xi$ while leaving the classically allowed energy range essentially unchanged. This is the precise sense in which increasing $\xi$ is a controlled, numerically realizable version of $\hbar\to0$.

\subsubsection{Valid window for $\xi$}
Any numerical spectrum is truncated at a finite highest computed level $E_{\max}$, so the plateau in $C_v(\xi)$ can only be trusted inside a finite window. For the level spacing to be negligible on the scale of $k_BT$ -- the condition for the sum over states to look like the continuous classical integral -- Eq.~\eqref{eq:bohrsommerfeld} requires
\begin{equation}
...
\label{eq:xiwindow}
\end{equation}
The lower bound is the semiclassical continuum condition of Eq.~\eqref{eq:bohrsommerfeld}; the upper bound is a purely numerical artifact of working with a finite basis (Section~\ref{sec:schottky} returns to this distinction).
...
Table~\ref{tab:params} lists the numerical control parameters governing the $\xi$/$n$-scan and the temperature sweep, using the notation introduced in Section~\ref{sec:xiconv}.

\begin{table}[!t]
\centering
\caption{Numerical Control Parameters of the $\xi$/$n$-Scan (Current Run)}
\label{tab:params}
\footnotesize
