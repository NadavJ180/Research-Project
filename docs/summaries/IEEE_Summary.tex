...
\label{sec:theory}
Throughout, the working substance is assumed to be in thermal equilibrium with a bath at temperature $T$: every $C_v(T)$ in this report is the equilibrium, canonical-ensemble heat capacity of a fixed spectrum $\{E_n\}$, not a finite-time or non-equilibrium quantity. This is the standard starting point for analyzing a quasi-static engine cycle built from a sequence of such equilibrium states; the finite-time corrections to that idealization are exactly the more advanced physics cited in Section~\ref{sec:intro}, and are beyond this project's scope.
...
For potentials with no analytical spectrum, $\{E_n\}$ is obtained from a Colbert--Miller sinc discrete variable representation (DVR) \cite{colbertmiller1992,lightcarrington2000}, which represents the wavefunction by its values on an evenly spaced grid rather than by coefficients in an analytical basis. The grid is faithful only if sampled finely enough relative to how quickly $\psi(x)$ oscillates, which the Nyquist--Shannon theorem turns into a condition on the local momentum of the state being represented: the grid spacing must resolve the shortest local de Broglie wavelength present in the wavefunction, or that high-momentum content aliases back into the low-lying part of the computed spectrum.
...
\label{eq:kinetic}
\end{equation}
This is an exact result (not an approximation) for the kinetic energy operator restricted to the space of band-limited functions sampled on that grid; the diagonal case follows from its own closed-form summation identity rather than by extrapolating the $i\neq j$ expression, which is singular -- not merely indeterminate -- at $i=j$. Because $T_{ij}$ in Eq.~\eqref{eq:kinetic} depends only on $|i-j|$, only $N$ distinct values need ever be evaluated -- the matrix's first row -- rather than the formula being evaluated independently at all $N^2$ entries; the full (still $N\times N$) Toeplitz matrix is then expanded from that row. The full Hamiltonian $H=T+V$ is then diagonalized with a banded/symmetric eigensolver that extracts only the lowest $N_{\text{lev}}$ eigenvalues directly (LAPACK's MRRR algorithm, via \texttt{eigvalsh} with an explicit index range), avoiding the cost of resolving the full, unneeded upper part of the spectrum.
...
The grid span and spacing are set automatically from the potential itself, rather than by hand. An energy ceiling $E_{\text{ceil}}\approx v_{\min}+1.5\,N_{\text{lev}}$ -- calibrated for the roughly-linear level spacing of an HO-like well, and only ever used as a starting estimate -- fixes a target grid extent via the classical turning points where $V(x)=E_{\text{ceil}}$, located by root-finding; the grid is padded modestly beyond them. The spacing is then set from the local momentum at that ceiling, $k_{\max}=\sqrt{2m(E_{\text{ceil}}-v_{\min})}/\hbar$, via $\Delta x = \pi/(2k_{\max})$ -- twice as fine as the bare Nyquist limit $\pi/k_{\max}$, a safety margin against the initial estimate being slightly optimistic. Because this first estimate is only a starting point, an adaptive loop then widens the span geometrically (holding $\Delta x$ fixed) and re-solves, comparing eigenvalues against the previous, narrower grid; once the change falls below a set tolerance, the span is declared converged. This corrects for anharmonic or multi-well potentials -- the double well among them -- whose wavefunction tails extend well beyond what the HO-calibrated initial estimate predicts.
