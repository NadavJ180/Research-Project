...
CHANGELOG (v1.0 -> v1.1)
---------------------------------------------------------------------
- RESOLUTION SEARCH reparameterised from num_points to dx:
    * `find_minimum_grid_points` now grows dx geometrically (starting
      from a fine, verified-passing dx_start) rather than shrinking
      num_points.  The function parameters `num_points_start` and
      `shrink_factor` are replaced by `dx_start` and `grow_factor`.
    * The primary return key is now `maximum_dx` (the coarsest grid
      spacing within tolerance) instead of `minimum_grid_points`.
      `minimum_grid_points` is still returned as a derived quantity.
    * Each trace entry now stores `dx` as its primary key (plus
      `num_points` for completeness).
    * `plot_grid_limit_search` now plots error vs dx on the x-axis
      (left = fine, right = coarse), with the boundary line at
      `maximum_dx`.  A secondary annotation on each point shows the
      corresponding num_points so nothing is hidden from the reader.
- LEVEL-COUNT SEARCH: no change to search logic.
    * `find_maximum_levels` now returns `dx_fixed` in its result dict
      (the constant dx of the fixed grid, derived from num_points and
      span). Used by `plot_level_limit_search`.
    * `plot_level_limit_search` now includes the fixed dx value in
      the subplot title, so results from both searches sit on the
      same physical footing when read side by side.
- `run_dvr_limit_analysis` docstring updated accordingly.
=====================================================================
"""
...
def plot_grid_limit_search(grid_result, system_name="System"):
    ...
    # Annotate each point with its num_points so the reader can cross-
    # reference with the level-limit plot (which uses num_points in title).
    for dx, err, n_pts in zip(dxs, errs, pts):
        ax.annotate(
            f"{n_pts}",
            xy=(dx, err),
