...
independently without this file needing structural changes:
    ...
    SECTION 6 -- DVR limit analysis (resolution &
                 level-count breakdown points)     (DVR_Limit_Finder_1_0)

Run this file directly (`python Quantum_HO_Master_1_1.py`) to execute
...
quantum/classical Cv(T) diagnostics, the final numerical-vs-
analytical Cv(T) + classical-limit benchmark with its own error curve,
and -- new in this version -- the two DVR breakdown-point searches
(minimum grid points for a fixed n, and maximum trustworthy n for a
fixed grid), each with its own error-vs-parameter plot.

CHANGELOG (v1.0 -> v1.1)
---------------------------------------------------------------------
- Added SECTION 6: wires up DVR_Limit_Finder_1_0.py for the HO system,
  using HO_Analytical_1_0's exact energies as the reference spectrum.
  Reuses the same fixed grid/span already built in Section 1, so the
  limit analysis directly answers "how much headroom does the grid I
  already used in Sections 1-5 actually have?"
- Updated the Section 3 import to HO_Energy_Level_Error_1_1 (renamed
  from 1_0 -- docstring-only change there, no behavior change).
=====================================================================
"""
...
from DVR_Limit_Finder_1_0 import run_dvr_limit_analysis
...
if __name__ == "__main__":
    ...
    BETA_MIN, BETA_MAX, N_BETA = 0.1, 50.0, 200
