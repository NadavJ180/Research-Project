...
# Import the core DVR function from your algorithm module
from DVR_algorithm import colbert_miller_dvr_1d

# ── Changes from version 1.2/1.3 to 1.4 ──────────────────────────────────────
# 1. Fixed HO spatial aliasing artifact by increasing grid density to satisfy the Nyquist limit.
# 2. Fixed low-T early-termination bug in converge_xi by replacing hardcoded atol=0.2 with an adaptive scale.
# 3. Increased Box grid density to isolate hard-wall Sinc truncation errors.
...
def converge_xi(energies, beta, xi_start, tol_xi, min_stable, xi_mult, max_steps):
    ...
    for step in range(max_steps):
        ...
        if step >= 2:
            ...
            # FIX: Adaptive collapse checking to prevent low-temperature early freeze-out trip-wire
...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------
    # System 1: 1-D Particle-in-a-Box (Numerical via DVR)
    # -------------------------------------------------------------------------
    # FIX: Expanded grid resolution to N=1200 points to push high-frequency 
    # hard-wall Sinc truncation reflections completely out of the 500 state block.
    GRID_POINTS_BOX = 1200
    L = np.pi
    box_pot = lambda x: np.zeros_like(x)
    dx_box = L / (GRID_POINTS_BOX + 1)
    
    print("Computing Box energy levels via Colbert-Miller DVR...")
    energies_box = colbert_miller_dvr_1d(
        potential_func=box_pot,
        num_levels=NUM_STATES,
        x_min=dx_box,
        x_max=L - dx_box,
        num_points=GRID_POINTS_BOX,
        mass=0.5,
        hbar=1.0
    )
    ...
    results_box = run(
        ...
        system_name="1-D Particle-in-a-Box (DVR Generated)",
    ...
    # -------------------------------------------------------------------------
    # System 2: 1-D Harmonic Oscillator (Numerical via DVR)
    # -------------------------------------------------------------------------
    # FIX: Expanded grid points to 1500. This achieves a spacing of dx = 0.06, 
    # which easily satisfies the Nyquist criteria (dx < 0.099) required to resolve
    # the high momentum oscillations at the bottom of the well for state n=500.
    GRID_POINTS_HO = 1500
    ho_pot = lambda x: 0.5 * x**2
    
    print("Computing Harmonic Oscillator energy levels via Colbert-Miller DVR...")
    energies_ho = colbert_miller_dvr_1d(
        potential_func=ho_pot,
        num_levels=NUM_STATES,
        x_min=-45.0,
        x_max=45.0,
        num_points=GRID_POINTS_HO,
        mass=1.0,
        hbar=1.0
    )
    ...
    results_ho = run(
        ...
        system_name="1-D Harmonic Oscillator (DVR Generated)",
        beta_min=BETA_MIN, beta_max=BETA_MAX, n_beta=N_BETA,
        xi_start=XI_START, tol_xi=TOL_XI, min_stable_xi=MIN_STABLE_XI,
        xi_multiplier=XI_MULT, max_xi_steps=MAX_XI_STEPS,
        tol_cv=TOL_CV, min_stable_n=MIN_STABLE_N,
